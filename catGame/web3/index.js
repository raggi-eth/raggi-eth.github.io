...
async function signData(message) {

  var from = await web3.eth.getAccounts();
  var epoch = message;
  var address = from[0].toLowerCase();
  var data = epoch + address;
  ...
  try {
    const datasignature = await web3.currentProvider.sendAsync(
      {
        method,
        params,
        from: address,
      ...
      function (err, result) {
        ...
        window.web3gl.signDataResponse = result.result;
    ...
    window.web3gl.signDataResponse = error.message;
  }
}
